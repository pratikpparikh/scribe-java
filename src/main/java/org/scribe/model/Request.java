...
public class Request {
  ...
  private static final Logger log = Logger.getLogger("com.scribe.model.Request");
  ...
  public Response send(RequestTuner tuner) {
    try {
      createConnection();
      return async ? doSendAsync(tuner) : doSend(tuner);
    } catch (Exception e) {
  ...
  Response doSendAsync(RequestTuner tuner) throws IOException {
    RequestBody body = null;// RequestBody.create(JSON, json);
    if (verb.equals(Verb.PUT)) {
    ...
    if (connectTimeout != null) {
      client.setConnectTimeout(connectTimeout.intValue(), TimeUnit.MILLISECONDS);
    }
    if (readTimeout != null) {
      client.setReadTimeout(readTimeout.intValue(), TimeUnit.MILLISECONDS);
    }
    if (writeTimeout != null) {
      client.setWriteTimeout(writeTimeout.intValue(), TimeUnit.MILLISECONDS);
    }
    tuner.tune(this);
  ...
  private void createConnection() throws IOException {
    if (log.isDebugEnabled()) {
      log.debug("Trying to establish connection started");
    }
    String completeUrl = getCompleteUrl();
    if (async && requestBuilder == null) {
      if (log.isDebugEnabled()) {
        log.debug("Async url is ".concat(completeUrl));
      }
      requestBuilder = new com.squareup.okhttp.Request.Builder().url(completeUrl);
      if (log.isDebugEnabled()) {
        log.debug("request builder initilaized ");
      }
    } else if (connection == null && !async) {
      if (log.isDebugEnabled()) {
        log.debug("sync url is ".concat(completeUrl));
      }
      System.setProperty("http.keepAlive", connectionKeepAlive ? "true" : "false");
      connection = (HttpURLConnection) this.factory.open(new URL(completeUrl));
      connection.setInstanceFollowRedirects(followRedirects);
      if (log.isDebugEnabled()) {
        log.debug("connection url initilaized ");
      }
    }
    if (log.isDebugEnabled()) {
      log.debug("Trying to establish connection ended");
    }
  }
   ...
   *
   * @see http ://docs.oracle.com/javase/6/docs/api/java/net/HttpURLConnection.html
